...
def mainloop():
    ...
    replacement_bgs = load_images(replacement_bgs, image_name,
                                  ...
                                  config.get(
                                      "background_interpolation_method"),
                                  image_filters)
    ...
    if config["dilate"]:
        mask = cv2.dilate(mask, np.ones(
            (config["dilate"], config["dilate"]), np.uint8), iterations=1)
    if config["erode"]:
        mask = cv2.erode(mask, np.ones(
            (config["erode"], config["erode"]), np.uint8), iterations=1)
    if config["blur"]:
